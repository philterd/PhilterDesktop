...
namespace PhilterDesktop
{
    ...
    public partial class EmailRedactionPreviewForm : Form
    {
        ...
        // inputs disabled and a wait cursor) so a large email never freezes the window.
        ...
        private void RefreshAll()
        {
            RefreshList();
            RefreshPreview();
            _save.Enabled = PolicyChosen && _fields.Length > 0;
        }
        ...
        private void OnSave(object? sender, EventArgs e)
        {
            ...
            // guard here too so the redacted email is never written without a detection pass.
            if (!PolicyChosen)
            {
                MessageBox.Show(this, "Choose a policy and a context before saving.", "Redact Email (Preview)",
            ...
            try
            {
                ...
                EmailRedactor.ApplySpans(_sourcePath, output, _spans, _settings.ScrubEmailHeaders, _settings.RemoveCommonEmailHeaders);
                stopwatch.Stop();

...
Because Philter Desktop runs entirely on the user's machine and stores its data locally (encrypted —
see [Settings → Security](settings.md#security-tab-protecting-your-stored-information)), there is no
server to stand up, no cloud account to manage, and no document data leaving your environment. The
official signed installer supports a standard per-user installation (no administrator rights required)
as well as an all-users installation; see [Getting Started](getting-started.md). For deployment
questions, support agreements, or anything else, contact Philterd through
**[philterd.ai](https://www.philterd.ai)**.

...
Redaction is a critical process for protecting sensitive information, but it is important to understand that it is not an exact science. While Philter Desktop makes use of sophisticated technologies to identify and mask sensitive data, the process is inherently probabilistic and can occasionally result in mistakes.
...
Philter Desktop uses a multi-layered approach to identification. The following two methods are primarily used to identify sensitive information:
...
When you identify mistakes in the automated redaction process, Philter Desktop provides several tools to help you remediate them and improve future performance.
...
*   **Terms to Never Redact**: If the system is over-redacting non-sensitive terms (like your company name or common industry terms) (False Positives), you can add them to a **Never Redact** (Always Ignore) list so they are left untouched.

### Global vs. Per-Policy Configuration

These lists can be configured at two different levels depending on your needs:

1.  **Global lists**: Terms on the global **Always Redact** and **Always Ignore** lists apply to **every** document you redact, no matter which policy you use — open them with the **Lists** button on the main toolbar. They are ideal for terms that should be handled consistently regardless of the specific document type or project (for example, your company's name or a project codename). See [Redaction Policies](policies.md) for details.
2.  **Per-Policy Terms**: You can also define **Always Redact** and **Always Ignore** (never redact) terms within an individual [redaction policy](policies.md). This is useful for project-specific terms that only apply to certain types of documents; they apply only when you redact with that policy.
...
Philter Desktop provides tools to assist in this review process, allowing you to quickly see what was changed and verify the results: automatic [verification](settings.md#verifying-redactions-automatically) re-scans the redacted output for anything the redaction may have left behind, the **redaction report** records what was removed by type and count, and **View Diff…** shows the source and redacted document side by side so you can see exactly what changed.

### Best Practices for Review

*   **Verify High-Risk Entities**: Pay extra attention to names, dates, and locations.
*   **Compare the documents**: Use **View Diff…** to review the exact text that was removed and confirm the system's logic aligned with your expectations, and use **Modify Redaction…** to add or remove redactions before saving.
*   **Iterate on Policies**: If you notice consistent mistakes, refine your [redaction policy](policies.md) or update your **Always Redact** / **Always Ignore** lists to improve future performance.

See also the [Best Practices](best-practices.md) page for more guidance on reviewing redacted documents.
